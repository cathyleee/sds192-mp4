...
budget_plot <- ggplot(data = gender_by_genre_crew, aes(x = reorder(genre, -budget), y = budget)) +
  geom_bar(stat = "identity") + 
  theme(axis.text.x = element_text(angle = 45)) +
  scale_x_discrete(name = "Genre") +
  scale_y_continuous(name = "Gross Revenue (dollars)", labels = scales::comma) 
...
figure <- ggarrange(gender_crew_plot, gender_acting_plot,
                    labels = c("Crew Gender by Genre", "Cast Gender by Genre"),
                    ncol = 2, nrow = 1)
figure

figure2 <- ggarrange(gross_plot, budget_plot,
