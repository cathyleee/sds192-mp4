...
```{r}
# among the cast, get the producers/writers/directors
# get proportion of men and women among producers/writers/directors
...
budget_with_genderprop <- budget_by_gender %>%
  drop_na(gender) %>%
  group_by(title) %>%
  summarize(count(gender)) 
```

```{r DISCARD}
# calculate the gender demogrpahics on the production team of each movie
budget_genderprop <- budget_by_gender %>%
  group_by(title, budget, gender) %>%
  summarize(num = n()) %>%
  drop_na(gender)

# calculate whether a movie's production team is female or male dominated
gender_domination <- budget_genderprop %>%
  mutate(dominant_gender = ifelse(num[gender == "m"] > num[gender == "f"], "male-dominated", "female-dominated"))
```

```{r}
# calculate the gender demographics on the production team of each movie
# calculate whether a movie's production team is female or male dominated 
budget_gender_prop <- budget_by_gender %>%
